...
~97% HCL spec complete. 778+ tests passing.
...
- **Collections** - `length`, `keys`, `values`, `contains`, `flatten`, `merge`, `reverse`, `distinct`, `sort`, `slice`, `element`
- **String** - `chomp`, `indent`, `join`, `lower`, `upper`, `replace`, `split`, `strrev`, `substr`, `trim`, `trimprefix`, `trimsuffix`, `trimspace`, `format`, `formatlist`
- **Type conversion** - `tobool`, `tonumber`, `tolist`, `tomap`, `toset`, `tostring`
...
| Category | Functions |
|----------|-----------|
| **Parse** | `parse(String) -> HCLResult[Body]` |
...
| **Eval** | `eval_expr(Expression, Map[String, HCLValue], Map[String, FuncDef]) -> HCLResult[HCLValue]`, `simplify_body(Body) -> Body`, `builtin_functions() -> Map[String, FuncDef]` |
