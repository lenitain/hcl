...
## 当前状态：源位置 Span (#10) 全部完成 ✅

## 当前分支
- 分支：`lenitain/feat/source-span`
- 状态：开发完成
- 目标：源位置 Span (#10)
...
### 源位置 Span (#10) ✅
- ✅ `Span` 结构体 — start_line, start_col, start_offset, end_line, end_col, end_offset
- ✅ `Span::new()`, `Span::empty()`, `Span::merge()` 方法
- ✅ `Spanned[T]` 泛型包装器 — value + optional span
- ✅ `Spanned::new()`, `Spanned::bare()`, `Spanned::unwrap()`, `Spanned::get_span()`
- ✅ `Attr.value` 类型从 `Expression` 改为 `Spanned[Expression]`
- ✅ `Attr::get_value()` 返回 unwrapped Expression
- ✅ `Attr::get_spanned_value()` 返回 Spanned[Expression]
- ✅ `Attr::set_span()` 设置 span
- ✅ `Body.span` 和 `Block.span` 可选字段
- ✅ Parser `parse_attr` 捕获 value 的 start/end 位置
- ✅ Parser `parse_body` 捕获 body 的 start/end 位置
- ✅ Parser `parse_block` 捕获 block 的 start/end 位置
- ✅ Visit/VisitMut 支持 Spanned[Expression]
- ✅ 14 个新测试（Span 类型 + Parser span 捕获）
- ✅ 所有 734 个测试通过
...
综合评估：整体约 93% 完整度。核心解析/求值/序列化已可用，仅剩类型统一 (#12)。
...
**当前工作**：源位置 Span (#10) - ✅ 全部完成（734 测试通过）
