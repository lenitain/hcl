...
## 当前分支
- 分支：`lenitain/feat/json-syntax-parsing`
- 状态：开发完成
- 目标：JSON 语法解析（P0 #3）

## 本次开发完成的任务
- ✅ `JsonValue` 中间类型（JsonNull/JsonBool/JsonNumber/JsonString/JsonArray/JsonObject）
- ✅ `JsonLexer` 词法分析器（token 识别、字符串转义、数字解析）
- ✅ `JsonParser` 递归下降解析器（解析 JSON 值、数组、对象）
- ✅ `JsonValue → Body` 映射（HCL JSON 等效语法规则）
- ✅ 原始值 → `Attr`（属性）
- ✅ JSON 对象 → `Block`（块）
- ✅ JSON 数组对象 → 多个 `Block`
- ✅ JSON 数组非对象 → `Attr` with `ExprArray`
- ✅ 标签嵌套：`{"label1": {"label2": {body}}}` → Block with labels
- ✅ `json_to_body` 公共 API
- ✅ `json_to_hcl_string` 公共 API
- ✅ `body_to_json()` 修复为 HCL JSON spec 合规（嵌套标签、重复块类型数组）
- ✅ `body_to_json_pretty()` 同步修复
- ✅ 20+7 个新测试覆盖所有场景（JSON 解析 + round-trip）
- ✅ 所有 635 个测试通过（相比之前增加 27 个）
- ✅ moon check: 0 errors
- ✅ moon fmt: 通过
- ✅ moon info: 通过
...
- ✅ `eval_add` — 支持 string+number, number+string, string+bool, bool+string 隐式转换
- ✅ `eval_eq/neq` — 支持 string↔number, string↔bool 隐式比较
- ✅ `eval_lt/gt/lte/gte` — 支持 string↔number, string↔bool 隐式比较
- ✅ `value_to_string` — 模板系统使用一致的转换函数（bool_to_string/number_to_string）
- ✅ 7 个新测试覆盖所有隐式转换场景
- ✅ 所有 595 个测试通过（相比之前增加 7 个）
- ✅ moon check: 0 errors
- ✅ moon fmt: 通过
...
- ✅ 所有 601 个测试通过（相比之前增加 6 个）
- ✅ moon check: 0 errors
- ✅ moon fmt: 通过

## 警告统计
- 28 警告（主要是 trait impl 的 unused_value，这些是 MoonBit trait 系统的正常警告）
...
**当前工作**：JSON 语法解析完成，下一步实现 Splat 运算符 (#6) + Strip markers (#7)
