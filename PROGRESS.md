...
- ✅ `regex(pattern, str)` — 查找第一个匹配，返回捕获组数组
  - 使用 `@string.Regex::new` + `execute`
  - 返回 `[full_match, group1, group2, ...]`
  - 无效 pattern → `HCLError::invalid_value`
- ✅ `regexall(pattern, str)` — 查找所有匹配
  - 使用 `@string.Regex::find` 迭代
  - 返回 `[[full, g1, g2], [full, g1, g2], ...]`
- ✅ `regex_replace(str, pattern, replacement)` — 替换所有匹配
  - 使用 `@string.Regex::replace_by`
  - 支持 `$N` 占位符替换为捕获组
- ✅ 9 个新测试（regex 3 + regexall 3 + regex_replace 3）

### 类型系统增强 TypeSystemEnhance ✅
- ✅ `TDynamic` 变体 — 新增到 `TypeName` 枚举
  - 与任意类型可统一（`TDynamic + X → Ok(X)`）
  - 转换为 dynamic 时直通（`(_, TDynamic) → Ok(v)`）
- ✅ 6 个新测试（TDynamic 与 number/string/array/dynamic/null 统一）
...
- 覆盖：内置函数（数字、集合、字符串、类型转换、集合论、编码、日期时间、网络、正则，共72个函数）
...
- 覆盖：正则函数（regex 匹配、regexall 全部匹配、regex_replace 替换、$N 占位符）
- 覆盖：TDynamic 类型（dynamic 与任意类型统一、convert passthrough）
...
- ✅ 正则函数 (regex, regexall, regex_replace)
- ✅ 通过 builtin_functions() 获取所有内置函数（72 个）
- ✅ `ParamType` 枚举 — Any, ParamBool, ParamNumber, ParamString, ParamArray, ParamObject, Nullable, OneOf
...
综合评估：整体约 99% 完整度。核心解析/求值/序列化/类型统一/正则已全部完成。
...
**当前状态**：~99% HCL 规范完整度（CryptoFuncs 因 MoonBit 无 crypto 库跳过）
...
| 表达式求值 | `eval_binary/unary`, 72 个内置函数 | ✅ |
...
🎉 全部 14 个迭代任务已完成！项目达到 99% HCL 规范完整度。
...
**剩余缺口评估：~1% 规范（CryptoFuncs 因生态限制跳过）**

| 缺口类型 | 数量 | 影响范围 | 开发难度 |
|----------|------|----------|----------|
| 内置函数缺失 | 5 (crypto) | 小（MoonBit 无 crypto 库） | N/A |
...
### 迭代 6：加密切密函数补齐 — CryptoFuncs ⏭️ 跳过

**状态**：⏭️ 跳过 — MoonBit 标准库无 SHA/MD5/crypto 模块
...
| 函数 | 签名 | MoonBit 支持 |
|------|------|-------------|
| `sha256(str)` | `(string) -> string` | ❌ 无 crypto 库 |
| `sha512(str)` | `(string) -> string` | ❌ 无 crypto 库 |
| `sha1(str)` | `(string) -> string` | ❌ 无 crypto 库 |
| `md5(str)` | `(string) -> string` | ❌ 无 crypto 库 |
| `bcrypt(str)` | `(string) -> string` | ❌ 无 crypto 库 |
...
**依赖**：MoonBit 核心 `@string.Regex` 模块（已内置）

| 函数 | 签名 | 实现要点 |
|------|------|----------|
| `regex(pattern, str)` | `(string, string) -> array(string)` | `Regex::new` + `execute`，返回捕获组数组 |
| `regexall(pattern, str)` | `(string, string) -> array(array(string))` | `Regex::find` 迭代所有匹配 |
| `regex_replace(str, pattern, replacement)` | `(string, string, string) -> string` | `Regex::replace_by` + `$N` 占位符 |
...
**改动文件**：`unify.mbt` + `unify_test.mbt`

| 特性 | 说明 | 难度 |
|------|------|------|
| `TDynamic` 类型 | 与任意类型可统一（`TDynamic + X → Ok(X)`） | ✅ 已完成 |
| `convert_value` 支持 | `(_, TDynamic) → Ok(v)` 直通 | ✅ 已完成 |
...
迭代 6:  CryptoFuncs         — ⏭️ 跳过 (MoonBit 无 crypto 库)
